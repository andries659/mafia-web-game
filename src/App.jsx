  ...
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .screenshot-frame {
      ...
      box-shadow:0 20px 60px rgba(0,0,0,0.6); transition:transform 0.3s, box-shadow 0.3s;
    }
    .screenshot-frame:hover {
      transform:translateY(-6px) scale(1.02);
      box-shadow:0 28px 80px rgba(0,0,0,0.8), 0 0 0 1px rgba(192,20,28,0.2);
    }
    ...
    .screenshot-dot { width:10px; height:10px; border-radius:50%; opacity:0.7; }
    .screenshot-bar {
    ...
    .feature-card:hover {
      border-color:rgba(192,20,28,0.3); transform:translateY(-4px);
      box-shadow:0 16px 48px rgba(0,0,0,0.5), 0 0 0 1px rgba(192,20,28,0.1);
    }
    .role-pill {
...
// ─── Screenshot frame ─────────────────────────────────────────────────────────
function ScreenshotFrame({ src, label }) {
  return (
...
// ─────────────────────────────────────────────────────────────────────────────
//  SCREENSHOTS
//  1. Drop your image files into  public/screenshots/
//  2. Update the src paths below to match your filenames
//  3. Update the label to whatever caption you want in the URL bar
// ─────────────────────────────────────────────────────────────────────────────
const SCREENSHOTS = [
...
function LandingPage({ onEnter }) {
  ...
  return (
    <div style={{ minHeight: '100vh', position: 'relative', overflowX: 'hidden' }}>
      ...
      {/* Ambient glows */}
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 0, background: 'radial-gradient(ellipse 80% 50% at 20% 10%, rgba(192,20,28,0.06) 0%, transparent 60%), radial-gradient(ellipse 60% 40% at 80% 80%, rgba(139,92,246,0.05) 0%, transparent 60%)' }} />

      {/* Nav */}
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, padding: '0 32px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(8,8,10,0.85)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 28, height: 28, borderRadius: 8, background: 'var(--blood)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 16px rgba(192,20,28,0.5)', color: 'white' }}>
            <SkullIcon size={14} />
          </div>
          <span className="font-display" style={{ fontSize: 17 }}>Mafia</span>
        </div>
      ...
      {/* Hero */}
      <section style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '120px 24px 80px', position: 'relative', zIndex: 1, textAlign: 'center' }}>
        {/* Blood drips */}
        <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '12vw', pointerEvents: 'none' }}>
          {[0,1,2].map(i => <div key={i} style={{ width: 1, background: 'linear-gradient(to bottom, var(--blood), transparent)', animation: `bloodDrip ${1.2+i*0.3}s ease ${i*0.2}s both`, height: `${80+i*30}px`, opacity: 0.3 }} />)}
        </div>

        <div className="fade-up" style={{ animationDelay: '0ms' }}>
          <span style={{ fontFamily: 'DM Mono', fontSize: 10, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'var(--blood)', background: 'rgba(192,20,28,0.1)', border: '1px solid rgba(192,20,28,0.25)', borderRadius: 100, padding: '5px 16px', display: 'inline-block', marginBottom: 32 }}>
            The Night Has Begun
          </span>
        </div>
        ...
        <h1 className="font-display fade-up" style={{ fontSize: 'clamp(52px, 10vw, 104px)', fontWeight: 900, fontStyle: 'italic', lineHeight: 0.9, letterSpacing: '-0.02em', marginBottom: 40, animationDelay: '140ms', background: 'linear-gradient(135deg, var(--blood) 0%, #ff4d55 50%, var(--blood) 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>One.</h1>

        <p className="font-body fade-up" style={{ fontSize: 'clamp(17px, 2.5vw, 22px)', color: 'var(--text-mid)', maxWidth: 560, lineHeight: 1.6, fontStyle: 'italic', marginBottom: 48, animationDelay: '200ms' }}>
        ...
        <div className="fade-up" style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center', animationDelay: '280ms' }}>
          <button className="btn-blood" style={{ padding: '14px 32px', fontSize: 12 }} onClick={onEnter}>Enter the Game</button>
        ...
        <div className="fade-up" style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', marginTop: 64, maxWidth: 640, animationDelay: '360ms' }}>
          {roles.map(r => (
            <span key={r.label} className="role-pill" style={{ color: r.color, borderColor: r.color+'30', background: r.color+'0a' }}>
        ...
        {/* Floating cards */}
        <div className="hide-mobile" style={{ position: 'absolute', right: 'max(40px, 8vw)', top: '50%', transform: 'translateY(-50%)', width: 90, height: 130, borderRadius: 14, background: 'linear-gradient(135deg, #16161f, #0e0c14)', border: '1px solid rgba(192,20,28,0.2)', boxShadow: '0 20px 60px rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'cardFloat 5s ease-in-out infinite', color: 'var(--blood)', opacity: 0.7 }}>
          <SkullIcon size={24} />
        </div>
        <div className="hide-mobile" style={{ position: 'absolute', left: 'max(40px, 8vw)', top: '45%', transform: 'translateY(-50%)', width: 76, height: 110, borderRadius: 12, background: 'linear-gradient(135deg, #16161f, #0e0c14)', border: '1px solid rgba(42,127,212,0.2)', boxShadow: '0 20px 60px rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'cardFloat2 6s ease-in-out infinite', color: 'var(--sapphire)', opacity: 0.5 }}>
          <SearchIcon size={20} />
        </div>
      ...
      {/* Screenshots */}
      <section id="screenshots" style={{ padding: '80px 0', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            <div className="section-divider" />
            <h2 className="font-display" style={{ fontSize: 'clamp(28px, 4vw, 42px)', marginBottom: 12 }}>The City Never Sleeps</h2>
            <p className="font-body" style={{ color: 'var(--text-mid)', fontSize: 18, fontStyle: 'italic' }}>Every night is a crime scene. Every day is a trial.</p>
          </div>
        </div>
        <div ref={trackRef} className="screenshots-track" onMouseDown={onMouseDown} onMouseMove={onMouseMove} onMouseUp={onMouseUp} onMouseLeave={onMouseUp}>
          {SCREENSHOTS.map((s, i) => <ScreenshotFrame key={i} src={s.src} label={s.label} />)}
        </div>
      ...
      {/* How to Play */}
      <section id="how-to-play" style={{ padding: '80px 24px', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 760, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            <div className="section-divider" />
            <h2 className="font-display" style={{ fontSize: 'clamp(28px, 4vw, 42px)', marginBottom: 12 }}>How It Works</h2>
            <p className="font-body" style={{ color: 'var(--text-mid)', fontSize: 18, fontStyle: 'italic' }}>Simple to learn. Impossible to master.</p>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', gap: 24, alignItems: 'flex-start', padding: '24px 0', borderBottom: i < 4 ? '1px solid var(--noir-border)' : 'none' }}>
                <div style={{ flexShrink: 0, width: 48, height: 48, borderRadius: 12, border: `1px solid ${step.color}30`, background: `${step.color}08`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <section style={{ padding: '80px 24px', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            <div className="section-divider" />
            <h2 className="font-display" style={{ fontSize: 'clamp(28px, 4vw, 42px)', marginBottom: 12 }}>Built for the Table</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
            {features.map((f, i) => (
              <div key={i} className="feature-card">
                <div style={{ width: 40, height: 40, borderRadius: 10, border: `1px solid ${f.accent}30`, background: `${f.accent}0c`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: f.accent, marginBottom: 16 }}>{f.icon}</div>
      ...
      <section style={{ padding: '80px 24px 120px', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 640, margin: '0 auto', textAlign: 'center' }}>
          <div style={{ background: 'var(--noir-card)', border: '1px solid rgba(192,20,28,0.2)', borderRadius: 24, padding: 'clamp(36px, 6vw, 64px)', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative' }}>
              <p style={{ fontFamily: 'DM Mono', fontSize: 10, letterSpacing: '0.25em', textTransform: 'uppercase', color: 'var(--blood)', marginBottom: 20 }}>The City Awaits</p>
      ...
      {/* Footer */}
      <footer style={{ borderTop: '1px solid var(--noir-border)', padding: '28px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, position: 'relative', zIndex: 1 }}>
        ...
        <p style={{ fontFamily: 'DM Mono', fontSize: 9, color: 'var(--text-dim)', letterSpacing: '0.1em' }}>THE CITY NEVER FORGETS</p>
      </footer>
    </div>
  );
}
...
function LoginPage({ onBack }) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
      ...
      <nav style={{ padding: '20px 28px', display: 'flex', alignItems: 'center', gap: 10, position: 'relative', zIndex: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 22, height: 22, borderRadius: 6, background: 'var(--blood)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 12px rgba(192,20,28,0.4)', color: 'white' }}><SkullIcon size={12} /></div>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px 24px', position: 'relative', zIndex: 1 }}>
        <div style={{ width: '100%', maxWidth: 420 }}>
          <div className="glass-card fade-up" style={{ padding: 'clamp(32px, 6vw, 52px)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 1, background: 'linear-gradient(to right, transparent, rgba(88,101,242,0.4), transparent)' }} />
            <div style={{ width: 64, height: 64, borderRadius: 18, background: 'rgba(88,101,242,0.1)', border: '1px solid rgba(88,101,242,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 28px', boxShadow: '0 8px 32px rgba(88,101,242,0.15)', color: '#5865F2' }}>
              <DiscordIcon size={28} />
            </div>
          ...
          <p style={{ textAlign: 'center', fontFamily: 'DM Mono', fontSize: 9, color: 'var(--text-dim)', letterSpacing: '0.08em', marginTop: 20, lineHeight: 1.7 }}>
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--noir-black)', flexDirection: 'column', gap: 16 }}>
      ...
      {status === 'loading' ? (
        ...
        <div style={{ background: 'var(--noir-card)', border: '1px solid rgba(192,20,28,0.3)', borderRadius: 16, padding: '32px 40px', textAlign: 'center', maxWidth: 380 }}>
